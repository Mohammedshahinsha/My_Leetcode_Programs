class Solution {
    public void solve(char[][] grid) {
        ...
        // Marking the unvisted '0's as 'X' that will be only in between bcz all the corner '0' and the 'O' which are attached with 'O' will me already be marked as true via DFS 
        for(int i=0; i< n; i++){
            for(int j=0; j< m; j++){
    ...
    public void dfs(int row, int col, int[] delrow, int[] delcol, boolean[][] visited, char[][] grid){
        ...
        for(int i=0; i< 4; i++){    // traversing 4 ways, up,right,down,left (neighbours)
            ...
            && !visited[nrow][ncol] && grid[nrow][ncol] == 'O'){
                dfs(nrow, ncol, delrow, delcol, visited, grid); // passing the neighbours i.r all 4 direction of curr cell to the DFS if it statisfies the condition so it will be marked true as soon as recc calls DFS
            }
        }
    }
}